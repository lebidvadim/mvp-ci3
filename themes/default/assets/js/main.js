(function ($) {
 "use strict";
	...
	$('#country-voip').change(function () {
		if($(this).val() !== 0)
		{
			$.ajax({
				url: '/modSip/ajax/country',
				type: "POST",
				...
				success: function(msg){
					...
					if(rez.status === 'yes')
					{
						$('#states-voip').html(rez.option).removeAttr('disabled').selectpicker({title: rez.title}).selectpicker('refresh');
					}
					if(rez.status === 'no')
					{
						$('#states-voip').html('').attr('disabled','disabled').selectpicker({title: rez.title}).selectpicker('refresh');
					}
				},
				beforeSend: function(){
					$('#states-voip').html('').attr('disabled','disabled').selectpicker({title: 'Загрузка...'}).selectpicker('refresh');
				}
			});
		}
	});
