...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->lang->load('ion_auth',$this->CI->router->lang);
		$this->load->helper(['cookie', 'language','url']);

		$this->load->library('session');

		$this->load->model('modAuth/ion_auth_model');
	...
	public function forgotten_password($identity)
	{
		...
		if ($user)
		{
			...
			if ($code)
			{
				...
				{
					$data['link'] = '<a href="'.site_url('recovery-ver/'. $data['forgotten_password_code']).'">'.__('Восстановления пароля').'</a>';
					$message = $this->parser->parse('modAuth/'.$this->config->item('email_templates', 'ion_auth') . $this->config->item('email_forgot_password', 'ion_auth'), $data, TRUE);
					$this->email->clear();
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		else
		{
			...
			{
				$data['link'] = '<a href="'.site_url('activate/'. $data['id'] .'/'. $data['activation']).'">'.__('Активировать ваш акаунт').'</a>';
				$message = $this->parser->parse('modAuth/'.$this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate', 'ion_auth'), $data, true);
	...
	public function get_user()
	{
		...
		if (!empty($user_id))
		{
			...
			$user->group = $this->ion_auth_model->get_users_groups()->row();
			return $user;
		}
		return NULL;
	}
